...
// ==============================================================================
// 🧠 الذاكرة الدائمة في الرام (RAM Cache) خارج React
// تعيش طوال بقاء التبويب مفتوحاً ولا تُحذف أبداً عند الخروج أو الرجوع للصفحة!
// ==============================================================================
...
const LOCAL_STORAGE_KEY = 'yr_persistent_cache_v4';

// استرجاع الكاش من التخزين المحلي لضمان وجودها حتى بعد تحديث الصفحة (F5)
try {
  const localSaved = localStorage.getItem(LOCAL_STORAGE_KEY);
  if (localSaved && !globalMemoryCache.isInitialized) {
    const parsed = JSON.parse(localSaved);
    globalMemoryCache.businesses = parsed.businesses || [];
    globalMemoryCache.banks = parsed.banks || [];
    globalMemoryCache.jobs = parsed.jobs || [];
    globalMemoryCache.properties = parsed.properties || [];
    if (globalMemoryCache.businesses.length > 0) {
      globalMemoryCache.isInitialized = true;
    }
  }
} catch {
  // كاش آمن
...
export const HomeView: React.FC<HomeViewProps> = ({
  ...
  // ⚡ تبدأ الصفحة فوراً بالبيانات المحفوظة في الرام (0.00 ثانية وبدون أي انتظار!)
  ...
  useEffect(() => {
    ...
    // إذا كانت البيانات محملة بالفعل مسبقاً في الرام، لا نزعج الزائر ولا نُفرغ الصفحة أبداً!
    async function syncDataSilently() {
      try {
        ...
        const newBusinesses = bRes.status === 'fulfilled' && bRes.value.data ? bRes.value.data : [];
        const newBanks = bnRes.status === 'fulfilled' && bnRes.value.data ? bnRes.value.data : [];
        const newJobs = jRes.status === 'fulfilled' && jRes.value.data ? jRes.value.data : [];
        const newProperties = pRes.status === 'fulfilled' && pRes.value.data ? pRes.value.data : [];

        // تحديث الذاكرة الدائمة
        if (newBusinesses.length > 0) {
          globalMemoryCache.businesses = newBusinesses;
          setLiveBusinesses(newBusinesses);
        } else if (businesses && businesses.length > 0 && globalMemoryCache.businesses.length === 0) {
          globalMemoryCache.businesses = businesses;
          setLiveBusinesses(businesses);
        }

        if (newBanks.length > 0) {
          globalMemoryCache.banks = newBanks;
          setLiveBanks(newBanks);
        }
        if (newJobs.length > 0) {
          globalMemoryCache.jobs = newJobs;
          setLiveJobs(newJobs);
        }
        if (newProperties.length > 0) {
          globalMemoryCache.properties = newProperties;
          setLiveProperties(newProperties);
        }
        ...
        localStorage.setItem(LOCAL_STORAGE_KEY, JSON.stringify({
          businesses: globalMemoryCache.businesses,
          banks: globalMemoryCache.banks,
          jobs: globalMemoryCache.jobs,
          properties: globalMemoryCache.properties
        }));
      } catch (err) {
  ...
  return (
    <div dir="rtl" className="space-y-5 pt-1 max-w-5xl mx-auto px-3 sm:px-4 font-['Cairo',sans-serif] text-white">
      ...
      {/* 1️⃣ قسم: آخر ما أضيف (فوري وبدون اختفاء عند الرجوع!) */}
...
print("✅ تم بنجاح تطبيق الذاكرة الدائمة في الرام (RAM Cache) بنسبة 100%.")
