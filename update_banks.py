    ...
    async function fetchBanks() {
      try {
        const resp = await fetch(
          `${SUPABASE_URL}/rest/v1/businesses?business_type=in.(BANK,WALLET,EXCHANGE)&select=*&order=created_at.desc`,
          { headers: { apikey: SUPABASE_KEY, Authorization: `Bearer ${SUPABASE_KEY}` } }
        );
        if (!resp.ok) throw new Error("فشل جلب قائمة البنوك");
        const rows = await resp.json();
        const validRows = rows.filter(b => b.status === "active" || b.is_published !== false);

        const revResp = await fetch(
          `${SUPABASE_URL}/rest/v1/reviews?select=entity_id,stars`,
          { headers: { apikey: SUPABASE_KEY, Authorization: `Bearer ${SUPABASE_KEY}` } }
        );
        ...
        banksList = validRows.map(b => {
          ...
          return {
            ...
            badge_type: b.verified_badge_type || (b.is_verified ? "gold" : "gray"),
