...
const CACHE_KEY = 'yr_home_live_cache_v3';
...
export const HomeView: React.FC<HomeViewProps> = ({
  ...
  // ⚡ استرجاع فوري من الذاكرة المحلية (0.01 ثانية) لمنع أي شاشة بيضاء
  ...
  const [liveBusinesses, setLiveBusinesses] = useState<any[]>(cachedData?.businesses || []);
  const [liveBanks, setLiveBanks] = useState<any[]>(cachedData?.banks || []);
  const [liveJobs, setLiveJobs] = useState<any[]>(cachedData?.jobs || []);
  const [liveProperties, setLiveProperties] = useState<any[]>(cachedData?.properties || []);
  ...
  useEffect(() => {
    ...
    async function fetchAllDataParallel() {
      try {
        // استدعاء متوازي لجميع الجداول دفعة واحدة (Promise.allSettled)
        ...
        if (newBusinesses.length > 0) setLiveBusinesses(newBusinesses);
        else if (businesses && businesses.length > 0) setLiveBusinesses(businesses);

        if (newBanks.length > 0) setLiveBanks(newBanks);
        if (newJobs.length > 0) setLiveJobs(newJobs);
        if (newProperties.length > 0) setLiveProperties(newProperties);

        localStorage.setItem(CACHE_KEY, JSON.stringify({
          businesses: newBusinesses.length > 0 ? newBusinesses : liveBusinesses,
          banks: newBanks.length > 0 ? newBanks : liveBanks,
          jobs: newJobs.length > 0 ? newJobs : liveJobs,
          properties: newProperties.length > 0 ? newProperties : liveProperties
        }));
      } catch (err) {
  ...
  return (
    <div dir="rtl" className="space-y-5 pt-1 max-w-5xl mx-auto px-3 sm:px-4 font-['Cairo',sans-serif] text-white">
      ...
      {/* 1️⃣ قسم: آخر ما أضيف (مباشر وحقيقي من Supabase) */}
      <div className="space-y-2.5">
        ...
        {recentlyAdded.length > 0 ? (
          ...
          <div className="p-6 rounded-2xl bg-[#0F1420] border border-zinc-800/80 text-center text-xs text-zinc-400">
            جاري مزامنة المنشآت المعتمدة من قاعدة البيانات...
          </div>
        )}
      </div>
      ...
      {/* 4️⃣ قسم: العقارات والاستثمار (Smart Empty State) */}
      <div className="space-y-2.5">
        <div className="flex items-center justify-between px-1">
          <h3 className="text-sm font-black text-white flex items-center gap-1.5">
          ...
          <div className="p-4 rounded-2xl bg-[#0F1420] border border-zinc-800/80 flex flex-col sm:flex-row items-center justify-between gap-3 text-center sm:text-right shadow-md">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-[10px] text-zinc-400 mt-0.5">كن أول من يضيف عقاره في المنصة ليصل إلى آلاف الزوار في عموم محافظات اليمن.</p>
              </div>
            </div>
            <button
              onClick={onNavigateRealEstate}
...
print("✅ تم كتابة وإغلاق HomeView.tsx بنجاح وبدون أي انقطاع.")
