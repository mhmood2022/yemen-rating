...
// 1. تحديث RatingsManager.tsx ليصفر المنشأة وجدول reviews معاً
let rmCode = fs.readFileSync('src/pages/admin/ratings/RatingsManager.tsx', 'utf8');

// تعديل دالة handleResetSingle لحذف المراجعات وتصفير المنشأة
rmCode = rmCode.replace(
  /const handleResetSingle = async [^{]+{([\s\S]*?)setBusinesses\(prev => prev\.map/,
  \`const handleResetSingle = async (bizId: string, bizName: string) => {
    setActionLoadingId(bizId);
    try {
      // 1. تصفير المنشأة في جدول businesses
      await supabase
        .from('businesses')
        .update({ rating: 0, review_count: 0, updated_at: new Date().toISOString() })
        .eq('id', bizId);

      // 2. حذف أي مراجعات مسجلة في جدول reviews لهذه المنشأة
      await supabase
        .from('reviews')
        .delete()
        .eq('entity_id', bizId);

      setBusinesses(prev => prev.map\`
);

// تعديل دالة handleResetAllBusinesses لتصفير كافة المنشآت وحذف كافة التقييمات
rmCode = rmCode.replace(
  /const handleResetAllBusinesses = async [^{]+{([\s\S]*?)await fetchData\(\);/,
  \`const handleResetAllBusinesses = async () => {
    if (!window.confirm('هل أنت متأكد من تصفير تقييمات كافة المنشآت في قاعدة البيانات؟ ستصبح جميعها 0.')) return;
    setLoading(true);
    try {
      await supabase
        .from('businesses')
...
fs.writeFileSync('src/pages/admin/ratings/RatingsManager.tsx', rmCode, 'utf8');
console.log('✅ تم ضبط RatingsManager.tsx لتصفير المنشأة وجدول التقييمات معاً.');

// 2. تحديث BusinessProfilePage.tsx لمزامنة التقييم بدقة تامة واللون الأسود وتسميات النجوم
const profileCode = \`import React, { useState, useEffect, useMemo } from 'react';
...
export const BusinessProfilePage: React.FC = () => {
  ...
  // مصدر التقييم الموحد: ما تحدده لوحة التحكم في قاعدة البيانات يظهر هنا تماماً
  const ratingValue = Number(business?.rating) || 0;
  const reviewCount = Number(business?.review_count) || reviews.length || 0;
  const hasRealRating = ratingValue > 0 && reviewCount > 0;

  // حساب أشرطة التوزيع بناء على نسبة التقييم الواقعية
  const distBars = useMemo(() => {
    if (!hasRealRating) return { 5: 0, 4: 0, 3: 0, 2: 0, 1: 0 };
    if (ratingValue >= 4.8) return { 5: 85, 4: 15, 3: 0, 2: 0, 1: 0 };
    if (ratingValue >= 4.0) return { 5: 60, 4: 30, 3: 10, 2: 0, 1: 0 };
    if (ratingValue >= 3.0) return { 5: 20, 4: 30, 3: 40, 2: 10, 1: 0 };
    return { 5: 10, 4: 10, 3: 20, 2: 30, 1: 30 };
  }, [hasRealRating, ratingValue]);

  // إرسال تقييم حقيقي لقاعدة البيانات وتحديث المنشأة
  const handleSubmitReview = async (e: React.FormEvent) => {
    e.preventDefault();
    if (userStars === 0) {
      setReviewStatus({ type: 'error', message: 'يرجى اختيار عدد النجوم للتقييم.' });
      return;
    }
    ...
    try {
      ...
      // تحديث معدل وعدد مراجعات المنشأة في قاعدة البيانات
      const newTotalCount = reviewCount + 1;
      const newAvgRating = Number(((ratingValue * reviewCount + userStars) / newTotalCount).toFixed(1));

      await supabase
        .from('businesses')
        .update({ rating: newAvgRating, review_count: newTotalCount })
        .eq('id', business.id);

      setBusiness((prev: any) => ({ ...prev, rating: newAvgRating, review_count: newTotalCount }));
      ...
      setReviewStatus({ type: 'error', message: 'تعذر حفظ التقييم، يرجى المحاولة لاحقاً.' });
    } finally {
  ...
  const getStarLabel = (s: number) => {
    switch (s) {
      ...
      case 2: return '⭐⭐ سيء';
      case 3: return '⭐⭐⭐ مقبول / متوسط';
      case 4: return '⭐⭐⭐⭐ جيد جداً';
      case 5: return '⭐⭐⭐⭐⭐ ممتاز ومثالي';
      default: return 'اختر التقييم بالضغط على النجوم';
    }
  };
  ...
  return (
    <main dir="rtl" className="w-full max-w-md md:max-w-3xl lg:max-w-4xl min-h-screen bg-black flex flex-col pb-12 select-none relative mx-auto md:px-4 font-['Cairo',sans-serif] text-zinc-100">
      ...
      {/* 1. غلاف المنشأة باللون الأسود الفاحم والهوية الرسمية (بدون كحلي) */}
      <div className="relative w-full h-44 sm:h-52 md:h-60 rounded-none md:rounded-2xl bg-gradient-to-br from-[#0B0F17] via-[#121824] to-[#070A10] flex items-center justify-center overflow-hidden mt-0 md:mt-1 shadow-lg border border-zinc-900">
        {business.cover_url ? (
        ...
        <button
          ...
          className="absolute top-3.5 right-3.5 z-20 w-9 h-9 rounded-full bg-black/60 hover:bg-black/80 backdrop-blur-md border border-white/15 text-zinc-300 hover:text-[#FFC500] flex items-center justify-center transition active:scale-90 shadow-md cursor-pointer"
        >
      ...
      {/* 3. بيانات المنشأة الأساسية بانسيابية تامة */}
      <div className="px-4 space-y-1.5 text-right">
        ...
        {/* سطر التقييم المتزامن تماماً مع لوحة التحكم والبطاقة */}
        <div className="flex items-center justify-start gap-1.5 text-[#FFC500] text-xs font-bold">
          ...
          {hasRealRating ? (
            <>
              <div className="flex text-[#FFC500]">
                {Array.from({ length: 5 }).map((_, i) => (
                  <Star key={i} className={\`w-3 h-3 \${i < Math.round(ratingValue) ? 'fill-[#FFC500]' : 'text-zinc-700'}\`} />
                ))}
              </div>
            ...
            <span className="text-zinc-500 font-normal text-[11px]">لا توجد تقييمات بعد</span>
          )}
        </div>
      ...
      <div className="px-4 pt-4">
        ...
        {/* 🌟 2. إعلان منتصف الصفحة */}
        <AdBanner placementId="2" className="my-4" />

        {/* جـ) تبويب التقييمات المتزامن 1:1 مع لوحة التحكم */}
        {activeTab === 'reviews' && (
          <div className="space-y-5">
            <div className="bg-[#0B0F17] p-5 rounded-2xl border border-zinc-800 space-y-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 items-center border-b border-zinc-800/80 pb-4">
                  <div className="text-right">
                    ...
                    <div className="flex items-baseline gap-1.5">
                      <span className="text-3xl font-black text-white">{ratingValue.toFixed(1)}</span>
                      <span className="text-zinc-500 text-sm font-bold">/ 5</span>
                    </div>
                    <div className="flex text-[#FFC500] my-1">
                      {Array.from({ length: 5 }).map((_, i) => (
                        <Star key={i} className={\`w-3.5 h-3.5 \${i < Math.round(ratingValue) ? 'fill-[#FFC500]' : 'text-zinc-700'}\`} />
                      ))}
                    </div>
                    <span className="text-xs text-zinc-400">{reviewCount} تقييم</span>
                  </div>
                      ...
                      <div key={s} className="flex items-center gap-2">
                        ...
                        <div className="flex-1 h-1.5 bg-zinc-800 rounded-full overflow-hidden">
                          <div
                            ...
                            style={{ width: \`\${(distBars as any)[s]}%\` }}
                          />
                        </div>
                        <span className="w-8 text-right font-medium text-[11px]">{(distBars as any)[s]}%</span>
                      </div>
                    ))}
                  </div>
                </div>
              ) : (
                <div className="text-center py-6 border-b border-zinc-800/80">
                  <p className="text-zinc-400 text-xs font-bold mb-1">لا توجد تقييمات بعد لهذه المنشأة</p>
                  <p className="text-zinc-600 text-[11px]">كن أول من يقيّم ويشارك تجربته!</p>
                </div>
              )}
              ...
              {/* نموذج إدخال التقييم: النجوم تبدأ مفرغة (0) مع التسميات التفاعلية */}
              {showReviewModal && (
                <form onSubmit={handleSubmitReview} className="bg-black p-4 rounded-xl border border-zinc-800 space-y-3 pt-3">
                  <div className="text-center space-y-1">
                    ...
                    <span className="text-[11px] font-bold text-[#FFC500] block mt-1">
                      {getStarLabel(userStars)}
                    </span>
                  </div>
                  ...
                  <button
                    ...
                    className="w-full py-2 bg-[#FFC500] text-black font-bold rounded-lg text-xs flex items-center justify-center gap-1 transition"
                  >
                  ...
                  <div key={i} className="p-3 bg-[#0B0F17] rounded-xl border border-zinc-800 text-right space-y-1">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        <div className="w-6 h-6 rounded-full bg-zinc-800 flex items-center justify-center text-zinc-400 text-xs font-bold">
                          {r.user_name?.charAt(0) || 'ز'}
                        </div>
                        <span className="text-xs font-bold text-white">{r.user_name}</span>
                      </div>
                      <div className="flex text-[#FFC500]">
                        {Array.from({ length: Number(r.stars || r.rating) || 5 }).map((_, si) => (
...
fs.writeFileSync('src/pages/templates/BusinessProfilePage.tsx', profileCode, 'utf8');
console.log('✅ تم توحيد ومزامنة التقييم واللون الأسود في BusinessProfilePage.tsx بنجاح.');
