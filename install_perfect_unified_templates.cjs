...
console.log('🚀 1. جاري كتابة قالب العرض الجماعي CategoryListing.tsx...');
...
console.log('🚀 2. جاري كتابة قالب العرض الفردي BusinessDetails.tsx...');
...
console.log('🚀 3. جاري تصحيح MainPublicApp.tsx...');
let mainCode = fs.readFileSync('src/MainPublicApp.tsx', 'utf8');
mainCode = mainCode.replace('const handleSelectCategory = (slug: string) => {', 'function handleSelectCategory(slug: string) {');
fs.writeFileSync('src/MainPublicApp.tsx', mainCode, 'utf8');

console.log('🚀 4. جاري ضبط vercel.json...');
const vercelConfig = {
  "cleanUrls": true,
  "rewrites": [
    { "source": "/category/:category", "destination": "/index.html?category=:category" },
    { "source": "/businesses", "destination": "/index.html" },
    { "source": "/business/:id", "destination": "/index.html?id=:id" },
    { "source": "/banks", "destination": "/banks.html" },
    { "source": "/bank/:slug", "destination": "/bank.html" },
    { "source": "/(.*)", "destination": "/index.html" }
  ]
};
fs.writeFileSync('vercel.json', JSON.stringify(vercelConfig, null, 2), 'utf8');

console.log('🚀 5. جاري الرفع المباشر إلى GitHub...');
execSync('git add .');
execSync('git commit -m "feat: complete unified collective and individual business templates"');
execSync('git push origin main');
console.log('🎉 تم بنجاح تام! تم الرفع إلى GitHub وسيقوم Vercel بالتحديث تلقائياً.');
