        ...
        const YRA = {
            ...
            bindLogin() {
                ...
                if (!form) { alert('❌ form غير موجود!'); return; }
                form.addEventListener('submit', async function(e) {
                    e.preventDefault();
                    const email = e.target.email.value;
                    const password = e.target.password.value;
                    const log = ['🔐 بدء الدخول: ' + email];

                    try {
                        log.push('1. تحميل Supabase...');
                        ...
                        if (error) { log.push('❌ signIn: ' + error.message); }
                        else {
                            log.push('✅ signIn نجح');
                            const { data: profile } = await sb.from('profiles').select('*').eq('id', data.user.id).single();
                            log.push('3. role=' + (profile ? profile.role : 'null'));
                            if (profile && (profile.role === 'admin' || profile.role === 'super_admin')) {
                                YR_DB.setSession({ id: profile.id, name: profile.full_name || 'الإدارة', email: profile.email, role: profile.role });
                                location.reload();
                                return;
                            } else { log.push('⚠️ role مرفوض'); }
                        }
                    } catch (err) { log.push('💥 ' + err.message); }

                    log.push('4. fallback localStorage...');
                    const users = YR_DB.get('users');
                    const user = users.find(u => u.email === email && u.password === YR_DB.hash(password) && u.role === 'admin');
                    if (user) { YR_DB.setSession(user); location.reload(); }
                    else { alert('🔍 تشخيص:\n' + log.join('\n')); self.showToast('بيانات غير صحيحة', 'error'); }
                });
            },
