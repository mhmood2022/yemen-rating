        ...
        document.querySelectorAll('[data-placement]').forEach(function(slot){
            ...
            if (ad) {
                ...
                if (ad.ad_type === 'adsense' && ad.content_html) {
                    ...
                    var isVideo = imgUrl.startsWith('data:video') || imgUrl.endsWith('.mp4') || imgUrl.endsWith('.webm');
                    slot.innerHTML = '<a href="' + href + '" target="_blank"' + clk + ' style="display:block;width:100%;height:100%;">' +
                        (isVideo ? '<video src="' + imgUrl + '" autoplay muted loop playsinline style="width:100%;max-height:180px;object-fit:cover;display:block;border-radius:12px;"></video>' : '<img src="' + imgUrl + '" alt="' + (ad.title || '') + '" style="width:100%;max-height:180px;object-fit:cover;display:block;border-radius:12px;">') +
                        '</a>';
                } else {
                    slot.innerHTML = '<a href="' + href + '" target="_blank"' + clk + ' style="color:var(--gold-400);font-weight:800;font-size:0.95rem;display:flex;align-items:center;justify-content:center;padding:16px;background:var(--navy-900);border:1px solid var(--border);border-radius:12px;gap:10px;text-decoration:none;"><i class="fa-solid fa-bullhorn"></i> ' + (ad.title || 'إعلان مميز') + '</a>';
                }
            } else {
