...
export const AppLayout: React.FC<AppLayoutProps> = ({ children, onNavigate }) => {
  return (
    <div className="min-h-screen flex flex-col bg-[#F7F8FA] dark:bg-[#000000] text-[#0F172A] dark:text-white transition-colors">
      ...
      <div className="flex-1 flex w-full max-w-7xl mx-auto bg-[#F7F8FA] dark:bg-[#000000]">
        <DesktopSidebar onNavigate={onNavigate} />
        <main className="flex-1 p-4 lg:p-6 w-full max-w-full overflow-hidden bg-[#F7F8FA] dark:bg-[#000000]">
          {children}
        </main>
      </div>

      <div className="w-full bg-white dark:bg-[#000000] pb-[calc(76px+env(safe-area-inset-bottom,0px))] lg:pb-0">
        <Footer />
      </div>
