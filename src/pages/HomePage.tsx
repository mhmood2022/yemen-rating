    ...
    <div className="min-h-screen bg-[#0D0D12] text-white p-4 md:p-8 dir-rtl">
      <header className="max-w-6xl mx-auto mb-8 text-center">
        ...
        <p className="text-gray-400 text-sm md:text-base">
          ابحث وقارن بين أفضل الشركات والمؤسسات في كافة المحافظات
        </p>
      </header>
      ...
      <main className="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredBusinesses.map((b) => (
          <div
            ...
            className="bg-[#14141C] border border-[#2A2A2A] rounded-2xl overflow-hidden flex flex-col justify-between hover:border-amber-400 transition cursor-pointer group shadow-lg"
          >
            <div>
              <div className="h-44 overflow-hidden relative">
                ...
                />
                <span className="absolute top-3 right-3 bg-black/80 text-amber-400 text-xs px-3 py-1 rounded-full border border-amber-400/30 font-bold">
                  {b.category_name}
                </span>
              </div>
              ...
              <div className="p-5">
                <div className="flex items-start justify-between mb-2">
                  <h3 className="font-bold text-lg text-white group-hover:text-amber-400 transition">{b.name}</h3>
                ...
                <p className="text-xs text-gray-400 mb-4 line-clamp-2">{b.description}</p>
            ...
            <div className="p-5 pt-0 flex items-center gap-2" onClick={(e) => e.stopPropagation()}>
              <button
                ...
                className={`flex-1 py-2.5 rounded-xl font-bold text-xs border ${
                  isCompared(b.id) ? 'bg-amber-400/20 text-amber-400 border-amber-400' : 'bg-[#0D0D12] text-gray-300 border-[#2A2A2A]'
                }`}
              >
