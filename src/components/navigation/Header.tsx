...
export const Header: React.FC<{ onNavigate?: (path: string) => void }> = ({ onNavigate }) => {
  ...
  return (
    <header className="sticky top-0 z-30 w-full h-[58px] bg-black border-b border-[#1C1C1C] px-3.5 flex items-center justify-between shadow-sm">
      {/* أقصى اليسار: جرس التنبيهات فقط */}
      <div className="flex items-center">
        <button
          type="button"
          onClick={() => onNavigate && onNavigate('/more')}
          className="w-9 h-9 rounded-full bg-[#111111] border border-[#222222] text-[#F5C400] hover:text-white flex items-center justify-center transition-colors relative"
      ...
      {/* أقصى اليمين: الشعار + النجمة المفرغة الأكبر + زر القائمة الجانبية في الطرف الأيمن */}
      <div className="flex items-center gap-2.5">
        ...
        {/* زر القائمة الجانبية في أقصى اليمين */}
        <button
          type="button"
          onClick={openMoreDrawer}
          className="p-1 rounded-lg text-white hover:text-[#F5C400] transition-colors pr-1"
