...
export const Header: React.FC<{ onNavigate?: (path: string) => void }> = ({ onNavigate }) => {
  ...
  return (
    <header dir="rtl" className="sticky top-0 z-30 w-full h-[58px] bg-black border-b border-[#1C1C1C] px-3.5 flex items-center justify-between transition-colors shadow-sm">
      {/* 1. اليمين: زر القائمة الجانبية + الشعار مع النجمة المفرغة الكبيرة */}
      <div className="flex items-center gap-2.5">
        <button
          type="button"
          onClick={openMoreDrawer}
          className="p-1 rounded-lg text-white hover:text-[#F5C400] transition-colors"
