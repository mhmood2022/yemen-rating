...
export const ExchangeRatesPage: React.FC<ExchangeRatesPageProps> = ({ onBack }) => {
  ...
  const [sanaaRates, setSanaaRates] = useState<{
    ...
    gold24: { name: 'ذهب عيار 24', buy: 42500, sell: 43500, change: '+0.40%', trend: 'up' },
    gold21: { name: 'ذهب عيار 21', buy: 37200, sell: 38100, change: '+0.35%', trend: 'up' },
    gold18: { name: 'ذهب عيار 18', buy: 31800, sell: 32500, change: '0.00%', trend: 'steady' },
  ...
  const [adenRates, setAdenRates] = useState<{
    ...
    gold24: { name: 'ذهب عيار 24', buy: 132000, sell: 135000, change: '+1.10%', trend: 'up' },
    gold21: { name: 'ذهب عيار 21', buy: 115500, sell: 118000, change: '+0.95%', trend: 'up' },
    gold18: { name: 'ذهب عيار 18', buy: 98000, sell: 101000, change: '-0.10%', trend: 'down' },
    goldPound: { name: 'جنيه ذهب إنجليزي', buy: 924000, sell: 945000, change: '+1.20%', trend: 'up' }
  });

  // محرك تقلب الكسور اللحظي الحي
  useEffect(() => {
  ...
  return (
    <div dir="rtl" className="max-w-6xl mx-auto px-3 sm:px-4 py-2 space-y-3 font-['Cairo',sans-serif] text-white">
      ...
      {/* تضمين الحركات */}
      <style>{`
        @keyframes yrArrowJumpUp { 0%, 100% { transform: translateY(0); } 50% { transform: translateY(-3px); } }
        @keyframes yrArrowJumpDown { 0%, 100% { transform: translateY(0); } 50% { transform: translateY(3px); } }
        @keyframes yrDigitTick { 0% { opacity: 0.7; } 100% { opacity: 1; } }
      ...
      <div className="flex items-center justify-between border-b border-[#1F2937] pb-2">
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <span className="text-[9px] text-[#9CA3AF] mt-0.5 block">
              تحديث مباشر للكسور والتقلبات اللحظية
            </span>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="w-7 h-7 rounded-lg bg-[#161619] border border-[#27272A] text-gray-300 hover:text-[#FFC500] flex items-center justify-center transition-all cursor-pointer shadow-sm active:scale-95"
            title="تحديث الأسعار"
          >
            <RefreshCw size={13} className={isRefreshing ? 'animate-spin text-[#FFC500]' : ''} />
          </button>
      ...
      <div className="flex items-center justify-between gap-2 bg-[#0F0F12] p-1.5 rounded-xl border border-[#222226] flex-wrap shadow-md">
        <div className="flex bg-[#161619] p-0.5 rounded-lg border border-[#27272A]">
          <button
            onClick={() => setActiveMarket('sanaa')}
            className={`px-3 py-1 rounded-md text-[11px] font-black transition-all flex items-center gap-1.5 cursor-pointer ${
        ...
        <div className="flex gap-1 bg-[#161619] p-0.5 rounded-lg border border-[#27272A]">
          <button
      ...
      {/* ============================================================
          تبويب 1: كروت العملات الرشيقة والمضغوطة (بدون ارتفاع زائد)
          ============================================================ */}
      {activeTab === 'rates' && (
        <div className="space-y-2.5">
          
          {/* عنوان القسم وتنبيه البث الحي */}
          <div className="flex items-center justify-between px-1">
            <span className="text-[11px] font-bold text-gray-300 flex items-center gap-1">
              <Activity size={12} className="text-[#FFC500]" /> العملات الأجنبية في {activeMarket === 'sanaa' ? 'صنعاء' : 'عدن'}
            </span>
            <span className="text-[9.5px] text-emerald-400 font-mono flex items-center gap-1">
              <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-ping" /> بث حي
            </span>
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
            
            {/* بطاقة الدولار الأمريكي — تصميم رشيق ومدمج بارتفاع مثالي */}
            <div className="bg-[#0F0F12] p-2.5 rounded-xl border border-[#222226] space-y-1.5 shadow-sm">
              <div className="flex justify-between items-center">
                <div className="flex items-center gap-1.5">
                  <span className="text-sm">{currentRates.usd.flag}</span>
                ...
                <span className={`px-1.5 py-0.2 rounded text-[9.5px] font-mono font-bold flex items-center gap-0.5 ${
                  ...
                  {currentRates.usd.trend === 'up' && <TrendingUp size={11} className="anim-tick-up" />}
                  {currentRates.usd.trend === 'down' && <TrendingDown size={11} className="anim-tick-down" />}
                  {currentRates.usd.trend === 'steady' && <Minus size={11} />}
                  <span>{currentRates.usd.change}</span>
                </span>
              </div>

              {/* سطر الشراء والبيع المدمج */}
              <div className="grid grid-cols-2 gap-1.5 pt-1 border-t border-[#1F2937] font-mono">
                <div className="px-2.5 py-1 rounded-lg bg-[#161619] border border-[#27272A] flex items-center justify-between">
                  <span className="text-[9px] text-gray-400 font-['Cairo']">شراء</span>
                  <div className="flex items-baseline gap-0.5">
                    <span className="text-xs sm:text-sm font-black text-[#16A34A] digit-ticker">
                ...
                <div className="px-2.5 py-1 rounded-lg bg-[#161619] border border-[#27272A] flex items-center justify-between">
                  ...
                  <div className="flex items-baseline gap-0.5">
                    <span className="text-xs sm:text-sm font-black text-[#DC2626] digit-ticker">
            ...
            {/* بطاقة الريال السعودي — تصميم رشيق ومدمج بارتفاع مثالي */}
            <div className="bg-[#0F0F12] p-2.5 rounded-xl border border-[#222226] space-y-1.5 shadow-sm">
              <div className="flex justify-between items-center">
                <div className="flex items-center gap-1.5">
                  <span className="text-sm">{currentRates.sar.flag}</span>
                ...
                <span className={`px-1.5 py-0.2 rounded text-[9.5px] font-mono font-bold flex items-center gap-0.5 ${
                  ...
                  {currentRates.sar.trend === 'up' && <TrendingUp size={11} className="anim-tick-up" />}
                  {currentRates.sar.trend === 'down' && <TrendingDown size={11} className="anim-tick-down" />}
                  {currentRates.sar.trend === 'steady' && <Minus size={11} />}
          ...
          {/* أسعار الذهب والسبائك — كروت مصغرة متناسقة */}
          <div className="space-y-1 pt-1">
            <span className="text-[11px] font-bold text-[#FFC500] block px-1 flex items-center gap-1">
              <Sparkles size={12} /> أسعار الذهب في {activeMarket === 'sanaa' ? 'صنعاء' : 'عدن'}
            </span>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-1.5 font-mono">
              {[
                ...
                <div key={idx} className="p-2 rounded-xl bg-[#0F0F12] border border-[#222226] space-y-0.5 shadow-sm">
                  <div className="flex justify-between items-center text-[9.5px]">
                    <span className="font-bold text-white font-['Cairo']">{g.title}</span>
                    <span className={`text-[8.5px] font-bold flex items-center gap-0.5 ${
                      g.data.trend === 'up' ? 'text-[#16A34A]' : g.data.trend === 'down' ? 'text-[#DC2626]' : 'text-[#FFC500]'
                    }`}>
                      {g.data.trend === 'up' && <TrendingUp size={9} className="anim-tick-up" />}
                      {g.data.trend === 'down' && <TrendingDown size={9} className="anim-tick-down" />}
                      {g.data.trend === 'steady' && <Minus size={9} />}
                      <span>{g.data.change}</span>
                    </span>
                  </div>

                  <div className="flex justify-between items-baseline text-[9px] pt-1 border-t border-[#1F2937]">
                    <span className="text-gray-400 font-['Cairo']">شراء:</span>
                    <div className="flex items-baseline gap-0.5">
                      <span className="font-black text-[#16A34A] digit-ticker">{g.data.buy.toLocaleString()}</span>
                  ...
                  <div className="flex justify-between items-baseline text-[9px]">
                    ...
                    <div className="flex items-baseline gap-0.5">
                      <span className="font-black text-[#DC2626] digit-ticker">{g.data.sell.toLocaleString()}</span>
                      <span className="text-[#FFC500] font-black text-[9px]">﷼</span>
                    </div>
                  </div>
                </div>
      ...
      {/* ============================================================
          تبويب 2: محول الصرف المباشر
          ============================================================ */}
      {activeTab === 'converter' && (
        <div className="bg-[#0F0F12] p-3.5 sm:p-4 rounded-xl border border-[#222226] space-y-3 shadow-xl">
          <div className="flex items-center gap-1.5 border-b border-[#222226] pb-2">
            <Calculator size={15} className="text-[#FFC500]" />
            <h3 className="text-xs sm:text-sm font-black text-white">محول الصرف المباشر وحساب الفوارق بين السوقين</h3>
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
            <div className="sm:col-span-2 space-y-1">
              <label className="text-[10px] text-gray-400 block font-bold">المبلغ المراد تحويله:</label>
              <div className="flex gap-1.5">
                <input
                  type="number"
                  value={calcAmount}
                  onChange={(e) => setCalcAmount(Number(e.target.value))}
                  className="flex-1 bg-[#18181C] border border-[#27272A] rounded-lg p-1.5 text-xs font-mono font-bold text-white outline-none focus:border-[#FFC500]"
                />
                <select
                  value={calcCurrency}
                  onChange={(e) => setCalcCurrency(e.target.value as any)}
                  className="bg-[#18181C] border border-[#27272A] rounded-lg px-2.5 py-1.5 text-xs font-bold text-white outline-none"
                >
            ...
            <div className="p-2 bg-[#18181C] rounded-lg border border-[#FFC500]/30 space-y-0.5 font-mono text-center">
              <span className="text-[9px] text-gray-400 font-['Cairo'] block">فارق الصرف بين عدن وصنعاء:</span>
              <div className="text-sm font-black text-[#FFC500]">
                +{converterResults.diff.toLocaleString()} <span className="text-[10px]">﷼</span>
              </div>
            </div>
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-2 gap-1.5 pt-1.5 border-t border-[#1F2937] font-mono">
            <div className="p-2.5 rounded-lg bg-[#161619] border border-[#27272A] flex items-center justify-between">
              <span className="text-[9.5px] text-gray-400 font-['Cairo']">صنعاء:</span>
              <div className="flex items-baseline gap-1">
                <span className="text-sm font-black text-white digit-ticker">{converterResults.sanaaTotal.toLocaleString()}</span>
                <span className="text-[#FFC500] font-black text-[10px]">﷼ يمني</span>
              </div>
            </div>
            <div className="p-2.5 rounded-lg bg-[#161619] border border-[#27272A] flex items-center justify-between">
              <span className="text-[9.5px] text-gray-400 font-['Cairo']">عدن:</span>
              <div className="flex items-baseline gap-1">
                <span className="text-sm font-black text-[#16A34A] digit-ticker">{converterResults.adenTotal.toLocaleString()}</span>
                <span className="text-[#FFC500] font-black text-[10px]">﷼ يمني</span>
              </div>
            </div>
          </div>
      ...
      {/* ============================================================
          تبويب 3: مقارنة البنوك والمحافظ
          ============================================================ */}
      {activeTab === 'banks' && (
        <div className="bg-[#0F0F12] p-3 sm:p-4 rounded-xl border border-[#222226] space-y-2 shadow-xl">
          <div className="flex justify-between items-center border-b border-[#222226] pb-1.5">
            <span className="text-xs font-bold text-white flex items-center gap-1.5">
              <Landmark size={14} className="text-[#FFC500]" /> مقارنة أسعار الصرف في البنوك والمحافظ
            </span>
          </div>
          ...
          <div className="overflow-x-auto no-scrollbar">
            <table className="w-full text-right text-xs">
              <thead className="bg-[#161619] text-[#9CA3AF] border-b border-[#27272A]">
                <tr>
                  <th className="py-2 px-2.5">الجهة المصرفية</th>
                  <th className="py-2 px-2.5 font-mono">الدولار (شراء/بيع)</th>
                  <th className="py-2 px-2.5 font-mono">السعودي (شراء/بيع)</th>
                  <th className="py-2 px-2.5 text-center">الرسوم</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-[#1F2937] text-white font-mono">
                {banksData.map((b, idx) => (
                  <tr key={idx} className="hover:bg-[#18181C]/50 font-['Cairo']">
                    <td className="py-2 px-2.5 font-bold flex items-center gap-1">
                      {b.type === 'بنك' ? <Landmark size={12} className="text-[#FFC500]" /> : <Wallet size={12} className="text-[#2EA5FF]" />}
                      <span>{b.name}</span>
                    </td>
                    <td className="py-2 px-2.5 font-mono">
                      <span className="text-[#16A34A]">{b.usdBuy.toFixed(2)}</span> / <span className="text-[#DC2626]">{b.usdSell.toFixed(2)}</span> <span className="text-[#FFC500] font-black text-[9px] mr-0.5">﷼</span>
                    </td>
                    <td className="py-2 px-2.5 font-mono">
                      <span className="text-[#16A34A]">{b.sarBuy.toFixed(2)}</span> / <span className="text-[#DC2626]">{b.sarSell.toFixed(2)}</span> <span className="text-[#FFC500] font-black text-[9px] mr-0.5">﷼</span>
                    </td>
                    <td className="py-2 px-2.5 text-center text-gray-400 font-mono">{b.fee}</td>
                  </tr>
                ))}
              </tbody>
            </table>
