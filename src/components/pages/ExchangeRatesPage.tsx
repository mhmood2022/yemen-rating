...
import { 
  ...
  Building2, Landmark, Wallet, Calculator, MapPin, 
  Clock, ArrowUpDown, ChevronDown, CheckCircle2, DollarSign
} from 'lucide-react';
...
export const ExchangeRatesPage: React.FC<ExchangeRatesPageProps> = ({ onBack }) => {
  ...
  // أسعار الصرف والذهب لأسواق صنعاء وعدن
  const ratesData = {
    sanaa: {
      marketName: 'أسواق صنعاء',
      usd: { name: 'الدولار الأمريكي', code: 'USD', flag: '🇺🇸', buy: 534.76, sell: 538.00, change: '-0.20%', isUp: false },
      sar: { name: 'الريال السعودي', code: 'SAR', flag: '🇸🇦', buy: 140.43, sell: 140.70, change: '+0.15%', isUp: true },
      gold24: { name: 'جرام عيار 24', buy: '42,500', sell: '43,500', change: '+0.40%', isUp: true },
      gold21: { name: 'جرام عيار 21', buy: '37,200', sell: '38,100', change: '+0.35%', isUp: true },
      gold18: { name: 'جرام عيار 18', buy: '31,800', sell: '32,500', change: '+0.30%', isUp: true },
      goldPound: { name: 'جنيه ذهب إنجليزي', buy: '298,000', sell: '305,000', change: '+0.50%', isUp: true },
      updatedAt: 'منذ دقيقة'
    },
    aden: {
      marketName: 'أسواق عدن',
      usd: { name: 'الدولار الأمريكي', code: 'USD', flag: '🇺🇸', buy: 1910.00, sell: 1925.00, change: '+0.80%', isUp: true },
      sar: { name: 'الريال السعودي', code: 'SAR', flag: '🇸🇦', buy: 501.50, sell: 504.00, change: '+0.65%', isUp: true },
      gold24: { name: 'جرام عيار 24', buy: '132,000', sell: '135,000', change: '+1.10%', isUp: true },
      gold21: { name: 'جرام عيار 21', buy: '115,500', sell: '118,000', change: '+0.95%', isUp: true },
      gold18: { name: 'جرام عيار 18', buy: '98,000', sell: '101,000', change: '+0.85%', isUp: true },
      goldPound: { name: 'جنيه ذهب إنجليزي', buy: '924,000', sell: '945,000', change: '+1.20%', isUp: true },
      updatedAt: 'منذ دقيقة'
    }
  };
  ...
  const converterResults = useMemo(() => {
    const sRate = calcCurrency === 'USD' ? ratesData.sanaa.usd.sell : ratesData.sanaa.sar.sell;
    const aRate = calcCurrency === 'USD' ? ratesData.aden.usd.sell : ratesData.aden.sar.sell;
  ...
  return (
    <div dir="rtl" className="max-w-6xl mx-auto px-3 sm:px-4 py-2 space-y-3.5 font-['Cairo',sans-serif] text-white">
      ...
      <div className="flex items-center justify-between border-b border-[#1F2937] pb-2.5">
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <span className="text-[9.5px] text-[#9CA3AF] mt-0.5 block">
              تحديث مباشر لأسعار العملات والذهب في صنعاء وعدن
            </span>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="w-8 h-8 rounded-xl bg-[#161619] border border-[#27272A] text-gray-300 hover:text-[#FFC500] flex items-center justify-center transition-all cursor-pointer"
            title="تحديث الأسعار"
          >
      ...
      {/* 3. شريط التبويبات والمناطق (صنعاء / عدن) */}
      <div className="flex items-center justify-between gap-2 bg-[#0F0F12] p-2 rounded-2xl border border-[#222226] flex-wrap">
      ...
      {/* ============================================================
          تبويب 1: أسعار العملات والذهب الفورية
          ============================================================ */}
      {activeTab === 'rates' && (
        <div className="space-y-3">
          
          {/* أسعار العملات الرئيسية (الدولار والسعودي) */}
          <div className="space-y-1.5">
            <div className="flex items-center justify-between px-1">
              <span className="text-xs font-bold text-gray-300">العملات الأجنبية في {currentRates.marketName}</span>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
              {/* الدولار */}
              <div className="bg-[#0F0F12] p-3 rounded-2xl border border-[#222226] space-y-2 shadow-sm">
                <div className="flex justify-between items-center">
                  ...
                  <span className={`px-2 py-0.5 rounded text-[10px] font-mono font-bold flex items-center gap-0.5 ${
                    currentRates.usd.isUp ? 'bg-[#16A34A]/20 text-[#16A34A]' : 'bg-[#DC2626]/20 text-[#DC2626]'
                  }`}>
                    {currentRates.usd.isUp ? <TrendingUp size={11} /> : <TrendingDown size={11} />}
                    {currentRates.usd.change}
                  </span>
                </div>
                ...
                <div className="grid grid-cols-2 gap-2 pt-1 border-t border-[#1F2937] font-mono">
                  <div className="p-2 rounded-xl bg-[#161619] border border-[#27272A]">
                    ...
                    <b className="text-sm sm:text-base font-black text-[#16A34A]">{currentRates.usd.buy.toFixed(2)} ﷼</b>
                  </div>
                  <div className="p-2 rounded-xl bg-[#161619] border border-[#27272A]">
                    <span className="text-[9px] text-gray-400 font-['Cairo'] block">سعر البيع</span>
                    <b className="text-sm sm:text-base font-black text-[#DC2626]">{currentRates.usd.sell.toFixed(2)} ﷼</b>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-[#0F0F12] p-3 rounded-2xl border border-[#222226] space-y-2 shadow-sm">
                <div className="flex justify-between items-center">
                  ...
                  <span className={`px-2 py-0.5 rounded text-[10px] font-mono font-bold flex items-center gap-0.5 ${
                    currentRates.sar.isUp ? 'bg-[#16A34A]/20 text-[#16A34A]' : 'bg-[#DC2626]/20 text-[#DC2626]'
                  }`}>
                    {currentRates.sar.isUp ? <TrendingUp size={11} /> : <TrendingDown size={11} />}
                    {currentRates.sar.change}
                  </span>
                </div>
                ...
                <div className="grid grid-cols-2 gap-2 pt-1 border-t border-[#1F2937] font-mono">
                  <div className="p-2 rounded-xl bg-[#161619] border border-[#27272A]">
                    <span className="text-[9px] text-gray-400 font-['Cairo'] block">سعر الشراء</span>
                    <b className="text-sm sm:text-base font-black text-[#16A34A]">{currentRates.sar.buy.toFixed(2)} ﷼</b>
                  </div>
                  <div className="p-2 rounded-xl bg-[#161619] border border-[#27272A]">
                    <span className="text-[9px] text-gray-400 font-['Cairo'] block">سعر البيع</span>
                    <b className="text-sm sm:text-base font-black text-[#DC2626]">{currentRates.sar.sell.toFixed(2)} ﷼</b>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="space-y-1.5 pt-1">
            <span className="text-xs font-bold text-[#FFC500] block px-1">أسعار الذهب والسبائك في {currentRates.marketName}</span>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 font-mono">
              {[
                ...
                <div key={idx} className="p-2.5 rounded-xl bg-[#0F0F12] border border-[#222226] space-y-1">
                  <div className="flex justify-between items-center text-[10px]">
                    <span className="font-bold text-white font-['Cairo']">{g.title}</span>
                    <span className="text-[9px] text-[#16A34A] font-bold">{g.data.change}</span>
                  </div>
                  <div className="flex justify-between text-[9.5px] pt-1 border-t border-[#1F2937]">
                    <span className="text-gray-400 font-['Cairo']">شراء: <b className="text-[#16A34A]">{g.data.buy} ﷼</b></span>
                  </div>
                  <div className="flex justify-between text-[9.5px]">
                    <span className="text-gray-400 font-['Cairo']">بيع: <b className="text-[#DC2626]">{g.data.sell} ﷼</b></span>
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="bg-[#0F0F12] p-4 sm:p-5 rounded-2xl border border-[#222226] space-y-3.5 shadow-xl">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 pt-2 border-t border-[#1F2937] font-mono">
            ...
            <div className="p-3 rounded-xl bg-[#161619] border border-[#27272A]">
              ...
              <b className="text-lg font-black text-[#16A34A]">{converterResults.adenTotal.toLocaleString()} ﷼ يمني</b>
            </div>
          </div>
        </div>
      )}
