...
export const ExchangeRatesPage: React.FC<ExchangeRatesPageProps> = ({ onBack }) => {
  ...
  return (
    <div dir="rtl" className="max-w-6xl mx-auto px-3 sm:px-4 py-2 space-y-3.5 font-['Cairo',sans-serif] text-white">
      ...
      {/* ============================================================
          تبويب 1: أسعار العملات والذهب مع رمز ﷼ بالأصفر الملكي الصافي
          ============================================================ */}
      {activeTab === 'rates' && (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
              ...
              <div className="bg-[#0F0F12] p-3 rounded-2xl border border-[#222226] space-y-2 shadow-md">
                ...
                <div className="grid grid-cols-2 gap-2 pt-1 border-t border-[#1F2937] font-mono">
                  <div className="p-2 rounded-xl bg-[#161619] border border-[#27272A] flex flex-col justify-between">
                    <span className="text-[9px] text-gray-400 font-['Cairo'] block">سعر الشراء</span>
                    <div className="flex items-baseline gap-1 mt-0.5">
                      <span className="text-sm sm:text-base font-black text-[#16A34A] digit-ticker">
                        {currentRates.usd.buy.toFixed(2)}
                      </span>
                      <span className="text-[#FFC500] font-black text-xs">﷼</span>
                    </div>
                  </div>
                  <div className="p-2 rounded-xl bg-[#161619] border border-[#27272A] flex flex-col justify-between">
                    <span className="text-[9px] text-gray-400 font-['Cairo'] block">سعر البيع</span>
                    <div className="flex items-baseline gap-1 mt-0.5">
                      <span className="text-sm sm:text-base font-black text-[#DC2626] digit-ticker">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 font-mono">
              {[
                ...
                <div key={idx} className="p-2.5 rounded-xl bg-[#0F0F12] border border-[#222226] space-y-1 shadow-sm">
                  ...
                  <div className="flex justify-between items-baseline text-[9.5px] pt-1 border-t border-[#1F2937]">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-2.5">
            ...
            <div className="p-3 bg-[#18181C] rounded-xl border border-[#FFC500]/30 space-y-1 font-mono text-center">
              ...
              <div className="text-base font-black text-[#FFC500]">
                +{converterResults.diff.toLocaleString()} <span className="text-xs">﷼</span>
              </div>
            </div>
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 pt-2 border-t border-[#1F2937] font-mono">
            <div className="p-3 rounded-xl bg-[#161619] border border-[#27272A] flex flex-col justify-between">
              <span className="text-[10px] text-gray-400 font-['Cairo'] block">قيمة المبلغ في أسواق صنعاء:</span>
              <div className="flex items-baseline gap-1 mt-1">
                <span className="text-lg font-black text-white digit-ticker">{converterResults.sanaaTotal.toLocaleString()}</span>
                <span className="text-[#FFC500] font-black text-xs">﷼ يمني</span>
              </div>
            </div>
            <div className="p-3 rounded-xl bg-[#161619] border border-[#27272A] flex flex-col justify-between">
              <span className="text-[10px] text-gray-400 font-['Cairo'] block">قيمة المبلغ في أسواق عدن:</span>
              <div className="flex items-baseline gap-1 mt-1">
                <span className="text-lg font-black text-[#16A34A] digit-ticker">{converterResults.adenTotal.toLocaleString()}</span>
                <span className="text-[#FFC500] font-black text-xs">﷼ يمني</span>
              </div>
            </div>
          </div>
                ...
                {banksData.map((b, idx) => (
                  <tr key={idx} className="hover:bg-[#18181C]/50 font-['Cairo']">
                    ...
                    <td className="py-2.5 px-3 font-mono">
                      <span className="text-[#16A34A]">{b.usdBuy.toFixed(2)}</span> / <span className="text-[#DC2626]">{b.usdSell.toFixed(2)}</span> <span className="text-[#FFC500] font-black text-[10px] mr-0.5">﷼</span>
                    </td>
                    <td className="py-2.5 px-3 font-mono">
                      <span className="text-[#16A34A]">{b.sarBuy.toFixed(2)}</span> / <span className="text-[#DC2626]">{b.sarSell.toFixed(2)}</span> <span className="text-[#FFC500] font-black text-[10px] mr-0.5">﷼</span>
                    </td>
