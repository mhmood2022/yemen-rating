...
export const SearchSection: React.FC<SearchSectionProps> = ({
  ...
  return (
    <section dir="rtl" className="w-full bg-gradient-to-b from-zinc-950 via-zinc-900 to-zinc-950 border-b border-zinc-800/80 py-4 sm:py-6 px-3 sm:px-6">
      <div className="max-w-6xl mx-auto space-y-3 sm:space-y-4">
        ...
        <div className="flex items-center justify-between flex-wrap gap-2">
          ...
          <span className="text-[10px] sm:text-xs text-zinc-400 flex items-center gap-1 bg-zinc-900/90 px-2.5 py-1 rounded-lg border border-zinc-800 shadow-sm">
            <Sparkles className="w-3.5 h-3.5 text-amber-400" />
            <span>22 محافظة يمنية • 26 تصنيفاً معتمداً</span>
          </span>
        </div>

        {/* The Themed Search Form */}
        <form onSubmit={handleFormSubmit} className="bg-zinc-950/95 border border-zinc-800/90 rounded-2xl p-2 sm:p-2.5 shadow-2xl flex flex-col md:flex-row items-stretch gap-2">
          ...
          <div className="flex-1 relative flex items-center">
            ...
            <input
              ...
              className="w-full bg-zinc-900/90 border border-zinc-800/90 rounded-xl pr-10 pl-4 py-2.5 sm:py-3 text-xs sm:text-sm text-white placeholder-zinc-500 focus:outline-none focus:border-amber-400 focus:ring-1 focus:ring-amber-400/40 transition-all"
            />
          </div>

          {/* 2. Custom Themed Governorate Dropdown (المحافظة) */}
          <div className="w-full md:w-52 relative group">
            <div className="absolute right-3.5 top-1/2 -translate-y-1/2 flex items-center gap-1 pointer-events-none text-amber-400">
            ...
            <select
              ...
              className="w-full appearance-none bg-zinc-900 border border-zinc-800 group-hover:border-zinc-700 rounded-xl pr-9 pl-8 py-2.5 sm:py-3 text-xs sm:text-sm text-zinc-100 font-medium focus:outline-none focus:border-amber-400 focus:ring-1 focus:ring-amber-400/40 cursor-pointer transition-colors shadow-inner"
            >
              ...
              {YEMEN_LOCATIONS.map((gov) => (
                <option
                  key={gov.id}
                  value={gov.id}
                  className="bg-zinc-950 text-zinc-100 py-2.5 hover:bg-zinc-900"
                >
                  {gov.name}
                </option>
              ))}
          ...
          {/* 3. Custom Themed City / District Dropdown (المدينة / المديرية) */}
          <div className="w-full md:w-48 relative group">
            <div className="absolute right-3.5 top-1/2 -translate-y-1/2 flex items-center gap-1 pointer-events-none text-zinc-400">
              <Navigation className="w-3.5 h-3.5 text-zinc-500 group-hover:text-amber-400" />
            </div>
            ...
            <select
              ...
              className="w-full appearance-none bg-zinc-900 border border-zinc-800 group-hover:border-zinc-700 rounded-xl pr-9 pl-8 py-2.5 sm:py-3 text-xs sm:text-sm text-zinc-100 font-medium focus:outline-none focus:border-amber-400 focus:ring-1 focus:ring-amber-400/40 disabled:opacity-40 disabled:bg-zinc-950/60 disabled:cursor-not-allowed cursor-pointer transition-colors shadow-inner"
            >
              <option value="all" className="bg-zinc-950 text-zinc-300 font-semibold py-2">
          ...
          <button
            type="submit"
            className="w-full md:w-auto px-6 py-2.5 sm:py-3 bg-amber-400 hover:bg-amber-300 active:scale-98 text-zinc-950 font-extrabold text-xs sm:text-sm rounded-xl transition-all flex items-center justify-center gap-2 shadow-lg shadow-amber-400/10 flex-shrink-0"
          >
        ...
        <div className="flex items-center gap-1.5 overflow-x-auto pb-1 text-xs text-zinc-400 scrollbar-none">
          ...
          {[
            ...
            { label: 'عقارات وشقق', query: 'عقارات' }
          ].map((tag) => (
            <button
              key={tag.label}
              type="button"
