...
export const BusinessCard: React.FC<Props> = ({
  ...
  // استخراج ميزات القطاع المفعلة فقط دون أي قيم وهمية
  const activeFeatures = useMemo(() => {
    const raw = (business as any).sector_features || 
                (business as any).sections_config?.features?.sector_features || 
                (business as any).features || [];
    if (Array.isArray(raw)) {
      return raw.filter((f: any) => f && f.enabled);
    }
    if (raw && typeof raw === 'object') {
      return Object.keys(raw)
        .filter(k => raw[k]?.enabled || raw[k] === true)
        .map(k => ({ label: raw[k]?.label || k }));
    }
    return [];
  }, [business]);
      ...
      <div className="p-4 md:p-5 flex flex-col justify-between">
        <div>
          <div className="flex justify-between items-start mb-1">
            <h3 className="text-base font-black text-white group-hover:text-amber-400 transition flex items-center gap-1.5 line-clamp-1">
              ...
              {business.is_verified && (
                <span className="inline-flex items-center gap-0.5 px-1.5 py-0.5 rounded text-[10px] font-bold bg-amber-400/10 text-amber-400 border border-amber-400/30">
          ...
          <div className="flex items-center gap-2 text-xs mb-2">
            {hasRating ? (
              <>
                ...
                <span className="text-neutral-400">({business.reviews_count || 0} تقييم)</span>
              </>
            ) : (
              <span className="text-neutral-500 text-[11px] font-medium">لا توجد تقييمات بعد</span>
            )}
          ...
          <div className="text-xs text-neutral-400 mb-2.5 flex items-center gap-1.5">
            ...
            <span>{business.city} {business.district ? `- ${business.district}` : ''}</span>
          </div>

          {/* ميزات القطاع المفعلة */}
          {activeFeatures.length > 0 && (
            <div className="flex flex-wrap gap-1.5 mb-3">
              {activeFeatures.slice(0, 3).map((feat: any, idx: number) => (
                <span
                  key={idx}
                  className="inline-flex items-center gap-1 px-2 py-0.5 rounded-lg bg-[#191924] border border-[#252535] text-zinc-300 text-[10px] font-medium"
                >
                  <i className="fa-solid fa-check text-amber-400 text-[8px]"></i>
                  <span>{feat.label}</span>
                </span>
              ))}
              {activeFeatures.length > 3 && (
                <span className="text-[10px] text-zinc-500 font-bold self-center">
                  +{activeFeatures.length - 3} ميزات
                </span>
              )}
            </div>
          )}
