...
export const VerifiedBadge: React.FC<VerifiedBadgeProps> = ({
  ...
  // الألوان الثلاثة المطابقة للصورة المرفقة
  const getColor = (v: VerifiedBadgeVariant) => {
    switch (v) {
      case 'blue':
        return '#1D9BF0'; // الأزرق المرجعي
      case 'gray':
      case 'silver':
        return '#8E8E93'; // الرمادي المرجعي
      case 'gold':
      case 'yellow':
      default:
        return '#F5C400'; // الأصفر/الذهبي المرجعي
    }
  };
  ...
  const getTitle = () => {
    ...
    return 'نشاط تجاري موثق رسمياً على يمن ريتغ';
  };

  return (
    <span
      className={cn('inline-flex items-center gap-1 select-none shrink-0', className)}
      title={getTitle()}
    >
      <svg
        width={size}
