...
export const VerifiedBadge: React.FC<VerifiedBadgeProps> = ({
  ...
  const getColor = (v: VerifiedBadgeVariant) => {
    switch (v) {
      case 'blue':
        return '#1D9BF0'; // الأزرق المعتمد
      case 'gray':
      case 'silver':
        return '#8E8E93'; // الرمادي المعتمد
      case 'gold':
      case 'yellow':
      default:
        return '#F5C400'; // الذهبي/الأصفر المعتمد
    }
  };
  ...
  const getTitle = () => {
    if (variant === 'blue') return 'شارة توثيق زرقاء (حساب معتمد)';
    if (variant === 'gray' || variant === 'silver') return 'شارة توثيق رمادية (جهة رسمية)';
    return 'شارة توثيق ذهبية (نشاط تجاري موثق)';
  };
