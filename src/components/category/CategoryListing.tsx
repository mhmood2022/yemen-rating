...
import { Star, MapPin, Phone, ArrowRight, Frown, Layers, CheckCircle2 } from 'lucide-react';
...
export const CategoryListing: React.FC<CategoryListingProps> = ({
  ...
  return (
    <div dir="rtl" className="space-y-6">
      ...
      {/* 🌟 شريط التصنيفات الفرعية البارز 🌟 */}
      {subcategories.length > 0 && (
        <div className="bg-zinc-900/80 border border-zinc-800 p-4 rounded-2xl space-y-2.5 shadow-lg">
          <div className="flex items-center gap-1.5 text-xs text-zinc-400 font-bold">
            <Layers className="w-3.5 h-3.5 text-amber-400" />
            <span>التصنيفات الفرعية في {category?.name}:</span>
          </div>
              ...
              <button
                ...
                className={`text-xs px-4 py-2 rounded-xl border font-medium transition-all ${
                  selectedSubCategory === sub.slug
                    ...
                    : 'bg-zinc-950 text-zinc-300 border-zinc-800 hover:border-amber-400/40 hover:text-white'
                }`}
              >
                {sub.name}
              </button>
      ...
      {filtered.length > 0 ? (
        ...
        <div className="text-center py-16 bg-zinc-900/30 border border-zinc-800 rounded-2xl p-6 space-y-3">
          ...
          <p className="text-xs text-zinc-400 max-w-sm mx-auto">
            لم نجد أي منشأة في هذا التصنيف الفرعي ضمن المحافظة المحددة.
          </p>
        </div>
      )}
